...
const std::string WINDOW_NAME = "Showcase1";
...
/**
 * @brief This function processes the input that is given to this specific window.
 * @tparam window The window that the information is passed to/from.
 */
...
/**
 * @brief This function creates a triangle VAO with the given vertices. The colors are decided on runtime.
 * @tparam vertices The vertices of the triangle.
 * @tparam size The size, usually sizeof(vertices).
 */
GLuint create_triangle_VAO(float* vertices, int size);
/**
 * @brief This function creates a triangle VAO with the given vertices. The colors are decided on creation and are given along with the vertices.
 * @tparam vertices The vertices of the triangle.
 * @tparam size The size, usually sizeof(vertices).
 * @tparam colors The color of the triangle.
...
int main(){
    ...
    if(alternative_fragments){
        program = create_shader_program("./res/vertex_shader1.glsl", "./res/fragment_shader1.glsl");
    }else{
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        if(alternative_fragments){
            ...
            if(alternative_colors){
                t = glfwGetTime();
                glUniform3f(color_location, GLfloat(0.5f + 0.5 * cos(3 * t)), GLfloat(0.5f - 0.5 * sin(2 * t)), 0.15f);
            }else{
        ...
        if(alternative_fragments){
            if(alternative_colors){
                t = glfwGetTime();
                glUniform3f(color_location, GLfloat(0.6f + 0.4 * cos(1 * t)), GLfloat(0.5f - 0.5 * sin(0.5 * t)), 0.5f);
            }else{
